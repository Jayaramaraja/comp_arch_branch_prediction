...
for index in range(len(walk_dir)):
	line_match = '.*'+str(bp_var)+'_'+str(test_true_name[index])+'_(\d+)BTBEntries_(\d+)local'
	resultFile = open(bp_var+"_"+test_true_name[index]+".csv",'wb')
	wr = csv.writer(resultFile, dialect='excel')
	wr.writerow(["BTBEntries", "localPred_size", "lookups", "BTBLookups", "Branches_not_in_BTB", "BTBHits", "BTBHitPct", "BTBMissPct", "CPI", "Branches", "predictedBranches", "BranchMispred", "BranchMispredPercent", "unpredictable", "predictable_pct", "unpredictable_pct"])
		...
		for filename in files:
			...
			if ( filename == 'stats.txt'):
				...
				if (line_matched):
					...
					#if ( (BTBEntries == 32768) | (BTBEntries == 8192) ):
						#continue
					localPredictorSize = float(line_matched.groups()[1])
					#if ( (localPredictorSize == 32768) | (localPredictorSize == 8192) ):
						#continue
					...
					with open(file_path) as f:
						for line in f:
							...
							if l2_miss_match:
								...
								print 'BTBEntries=',BTBEntries
								print 'localPredictorSize=',localPredictorSize
								print 'unpredictable_pct =',unpredictable_pct,'\n\n'
								wr.writerow([BTBEntries,localPredictorSize,lookups,BTBLookups,branches_not_stored_in_BTB,BTBHits,BTBHitPct,BTBMissPct,cpi,Branches,predictedBranches,BranchMispred,BranchMispredPercent,unpredictable,predictable_pct,unpredictable_pct])
